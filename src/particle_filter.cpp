...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	  /**
	   * Suggested Steps From Udacity:
	   * (1) Set the number of particles. Initialize all particles to 
	   *   first position (based on estimates of x, y, theta and their uncertainties
	   *   from GPS) and all weights to 1. 
	   * (2) Add random Gaussian noise to each particle.
...
void ParticleFilter::prediction(double delta_t, double std_pos[],
	...
	// This is another way to approach adding the noise separate from calculating the target/mean
	...
	for (int i = 0; i < num_particles; ++i) {
		...
		std::normal_distribution<double> meas_with_noise_x(movement_x, std_pos[0]*1);    //T-shoot only - experimented with increasing the standard deviation at update step
		...
		// Here is the corresponding alternate method - if the random noise is zero mean
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted,
	...
	/**
	 * Suggested Steps From Udacity:
	 * (1) Find the predicted measurement that is closest to each
	...
	//NOTE - Not used currently -  THIS IS EMBEDDED DIRECTLY IN MY updateWeights Function/Method
}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  /**
   * Suggested Steps From Udacity:
   *   (1) Update the weights of each particle using a mult-variate Gaussian 
   ...
   */
	...
	bool allow_one_adjustment_print_per_pset_flag = true;  //set to true for each particle set, but only prints if adjustment made to sensor_goodness
	...
	for (int i = 0; i < num_particles; ++i) {
		...
		// make a temp struct to store transformed x,y particle point of view observations
		// this isn't actually needed here but probably would be if I separated this out into its own function as suggested
		// in the dataAssociation function - since it takes in LandmarkObs vectors
		...
		for (int j = 0; j < observations.size(); ++j) {
			...
			particles[i].sense_y.push_back(particles[i].y + (sin(particles[i].theta)*observations[j].x) + (cos(particles[i].theta)*observations[j].y));  //June-12 and June-13 found error here
		...
		for (int ja = 0; ja<particles[i].associations.size(); ++ja){
			...
			// calculate exponent -- using formulas from Udacity lesson
			...
			if (sense_goodness < 0.000000000000000000000001) {
				sense_goodness = 0.000000000000000000000001;
				
				if (allow_one_adjustment_print_per_pset_flag) { //print out only for first couple of particles
					//std::cout << "Insert lower-limit On sensor goodness Value for some measurements for this particle" << std::endl;
					allow_one_adjustment_print_per_pset_flag = false;
				}
			}
	...
	//FIXME - this is somewhat redundant with what I'm trying to do in the "sensor goodness" adjustment if its a really small value
	// probably want to remove this or make it more clear what's happening here and the interaction
